...
XToLevel = { }
XToLevel.version = "2.5.1_62"
XToLevel.releaseDate = '2021-06-10T16:49:06Z'

XToLevel.frame = CreateFrame("FRAME", "XToLevel", UIParent)
XToLevel.frame:RegisterEvent("PLAYER_LOGIN")

-- Register slash commands at load time so they are always available,
-- regardless of player level or initialization state.
SLASH_XTOLEVEL1 = "/xtolevel";
SLASH_XTOLEVEL2 = "/xtl";
SlashCmdList["XTOLEVEL"] = function(arg1)
    if XToLevel.OnSlashCommand then
        XToLevel:OnSlashCommand(arg1)
    else
        DEFAULT_CHAT_FRAME:AddMessage("|cFFaaaaaaXToLevel:|r Addon is not active (max level reached or initialization failed).")
    end
end
